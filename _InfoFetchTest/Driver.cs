...
namespace InfoFetch
{
    public static class Driver
    {
        ...
        public static bool CheckChromeDriver()
        {
            ...
            {
                Notification.Push(@"Chrome驱动", "未找到chromedriver");
                driverOK = false;
            }
            else if(!CheckChromeDriverVersion())
            {
                Notification.Push(@"Chrome驱动", "chromedriver版本不正确");
                driverOK = false;
            }
            if(!driverOK)
            {
        ...
        {
            Notification.Push(@"Chrome驱动", @"正在下载chromedriver");

            string webContent;
            if(File.Exists(localHtmlFile))
            {
                StreamReader sr = new StreamReader(localHtmlFile);
            ...
            File.SetAttributes("chromedriver.exe", FileAttributes.Normal);
        ...
        private static bool CheckChromeDriverVersion()
        {
            ...
            {
                Notification.Push(@"Chrome驱动", @"未知chromedriver版本");
                return false;
            }
        ...
        public static void UpdateLocalHtml()
        {
            ...
            File.WriteAllText("chromedriver.html", content);
        ...
        public const string localHtmlFile = @"chromedriver.html";
    }
}
